...
$css = $css + "BODY{background-color:white; font-family:Calibri;}"
$css = $css + "TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;}"
$css = $css + "TH{border-width: 1px;padding: 0px;border-style: solid;border-color: black;background-color:white}"
$css = $css + "TD{border-width: 1px;padding: 0px;border-style: solid;border-color: black;background-color:white}"
$css = $css + "</style>" 


#Table Declarations
$servers = @(LIST YOUR SERVERS HERE OR SWAP WITH A GET-CONTENT OR IMPORT-CSV)
$diskInfo = @()
$report = @()

#Intial Loop to apply to each server 
foreach($server in $servers)
{

    $diskinfo = Get-WmiObject Win32_Volume -Filter "DriveType='3'" -ComputerName $server | where{$_.Name -notlike "\\?*"} | ForEach {
                    New-Object PSObject -Property @{
                    Drive = $_.Name
                    Computer = $server
                    Percentage = (100 - ([Math]::round((($_.FreeSpace/$_.Capacity)*100))))
                }
            } 
    #Loops and checks if Disk above 90% usage 
    Foreach($disk in $diskInfo)
    {
        if($disk.Percentage -ge "90") 
        {
         $report += [pscustomobject]@{
                "Server Name" = $server
                Status = "Warning"
                Comment = "Disk " + $disk.Drive + " Capactiy Over 90%"
                }
        }
        #Loops and checks if Disk above 80% usage and below 90%
        if(($disk.Percentage -ge "80") -and ($disk.Percentage -lt "90"))
        {
         $report += [pscustomobject]@{
                "Server Name" = $server
                Status = "Caution"
                Comment = "Disk " + $disk.Drive + " Capactiy Over 80%"
                }
        }
        #Loops and checks if Disk less 80% usage 
        if($disk.Percentage -lt "80") 
        {
         $report += [pscustomobject]@{
                "Server Name" = $server
                Status = "Ok"
                Comment = "Disk " + $disk.Drive + " Under threshold"
                }
        }
...
#Converts Report to HTML and sets colouring of each cell based on Status
$report | ConvertTo-Html -Property 'Server Name','Status','Comment' -Head $css -Body "<h1>Health Check Report</h1>" | Set-CellColor -Property Status -Color Green -Filter "Status -like 'ok' " | Set-CellColor -Property Status -Color orange -Filter "Status -like 'Caution'" | Set-CellColor -Property Status -Color Red -Filter "Status -like 'Warning'" |  Out-File C:\Temp\test.html

$reportBody = Get-Content C:\Temp\test.html

#Send-MailMessage -Body ($reportBody | Out-String) -BodyAsHtml 